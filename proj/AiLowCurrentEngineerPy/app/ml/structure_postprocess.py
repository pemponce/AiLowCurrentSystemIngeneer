...
def _repair_wall_gaps(wall_mask: np.ndarray) -> np.ndarray:
    ...
    k_wide_h = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 25))
    ...
    k_wide_v = cv2.getStructuringElement(cv2.MORPH_RECT, (25, 1))
...
def _find_plan_bbox(wall_mask: np.ndarray) -> tuple[int, int, int, int]:
    ...
    for i in range(1, num):
        ...
        # Пропускаем мелкий шум
        if area < 500:
            continue

        # Пропускаем тонкие линии (граничные линии чертежа, выноски)
        if bw < 5 or bh < 5:
            continue

        # Пропускаем компоненты на всю высоту или ширину (рамки листа)
        if bh > h * 0.8 or bw > w * 0.8:
            continue
    ...
    if not found or x_max <= x_min or y_max <= y_min:
        return (0, 0, w, h)

    # Паддинг чтобы не обрезать комнаты у самого края bbox
    pad   = 60
    ...
    # Уточняем правую границу: ищем самую правую стену по центру высоты плана.
    # Это отсекает подъезд/улицу справа если правая стена квартиры не доходит
    # до края bbox (модель не предсказала её полностью как wall).
    mid_y = (y_min + y_max) // 2
    for x in range(x_max - 1, x_min, -1):
        if wall_mask[mid_y, x] > 0:
            x_max = min(x_max, x + pad)
            break
...
def _extract_rooms(wall_mask, door_mask, win_mask=None, min_room_area=5000):
    h, w = wall_mask.shape
    ...
    dilated = cv2.dilate(combined, k, iterations=2)
    free_space = cv2.bitwise_not(dilated)

    # Убираем 5px рамку по краю изображения
    border_mask = np.zeros((h, w), dtype=np.uint8)
    border_mask[5:-5, 5:-5] = 255
    free_space = cv2.bitwise_and(free_space, border_mask)

    # Ограничиваем область поиска bounding box-ом стен квартиры.
    # Всё снаружи bbox — улица, лестничная клетка, не наша территория.
    bx, by, bw_plan, bh_plan = _find_plan_bbox(wall_mask)
    plan_mask = np.zeros((h, w), dtype=np.uint8)
    plan_mask[by:by+bh_plan, bx:bx+bw_plan] = 255
    free_space = cv2.bitwise_and(free_space, plan_mask)

    num, labels, stats, centroids = cv2.connectedComponentsWithStats(free_space, connectivity=8)

    # Фон — компонент в точке верхнего края изображения (там точно снаружи)
    bg_id = int(labels[2, w//2])
    if bg_id == 0:
        bg_id = int(labels[h//2, 2])

    candidates = [
        (i, int(stats[i, cv2.CC_STAT_AREA]))
        for i in range(1, num)
        if int(stats[i, cv2.CC_STAT_AREA]) >= min_room_area and i != bg_id
    ]
...
def extract_geometry(
    ...
    rooms         = _extract_rooms(wall_bin, door_bin, win_mask=win_bin,
